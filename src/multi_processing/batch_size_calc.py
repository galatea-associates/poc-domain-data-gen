...
# Set of utility methods used to determine what batch size should be used for jobs
#
...
# 
# Where domain objects are not, i.e, the amount to generate is based on randomness
#   and prior generated domain objects, calculate a batch size to retrieve from
#   the dependency database a number of rows which would reasonably result in an
#   amount of domain objects close to the default generation_job_size_value
...
def swap_position_size(custom_args, target_num_records):
    ...
    batch_size = math.ceil((2*target_num_records)/(3*num_dates*(ins_min+ins_max)))
    return batch_size
